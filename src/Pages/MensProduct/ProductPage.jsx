        ...
        return keys.map((el) => {
          if (el.title === "Category") {
            ...
            return item.variant.map((c) => {
              return (
                el.keys.toUpperCase() === c.color.toUpperCase() &&
                resData.push(item)
              );
            });
            ...
            <div className={styles.head_right}>
              ...
              >
                <div className={styles.featured}>
                  <div className={`${styles.filter_btn} ${styles.width} `}>
                    <div className={`${styles.filter_btn_left} `}>
                      {upperFilterProperties.map((el) => {
                        return el.status && <p key={el.id}>{el.value}</p>;
                      })}
                    </div>
                    <div className={styles.filter_btn_icon}>
                      {upperFilters ? (
            ...
            {showFilters && (
              <div className={`${styles.filters_panel}`}>
                {filters.map((item, index) => {
                  return (
                    <div key={item.id} className={styles.accordian}>
                      <div
                      ...
                      >
                        ...
                        <div className={styles.icons}>
                          {item.status ? <RemoveIcon /> : <AddIcon />}
                        </div>
                      </div>
                      {item.status && (
                            ...
                            return (
                              <label key={el.id}>
                                <input
                                  ...
                                  type="checkbox"
                                  onChange={() =>
                                    handleFilterChange(el.key, index, i)
                                  }
                                  value={el.key}
                                />
                                {el.value}
